...
wrSVfix <- function() {
  glue::glue('### Useful stuff \n',
             ...
             '  TopMarkers1 <- DEG %>% dplyr::filter(padj==0) %>% dplyr::group_by(group) %>% dplyr::top_n(n=top_gene,wt=logFC) \n',
             '  TopMarkers2 <- DEG %>% dplyr::filter(padj<0.01) %>% dplyr::group_by(group) %>% dplyr::top_n(n = top_gene, wt = logFC) \n',
             '  TopMarkers <- rbind(TopMarkers1,TopMarkers2) %>% unique.matrix() %>% dplyr::arrange(group) \n',
             '  return(TopMarkers) \n',
             '}} \n',
             ' \n',
