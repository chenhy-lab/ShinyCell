...
wrSVfix <- function() {
  glue::glue('### Useful stuff \n',
             ...
             'names(cList) = c("Lightgrey-Blue", "White-Red", "Blue-Yellow-Red", "Yellow-Green-Purple") \n',
             ' \n',
             'Diff_table <- data.frame(feature=NA,group=NA,avgExpr=NA,logFC=NA,pval=NA,padj=NA,pct_in=NA,pct_out=NA) \n',
             'use_data <- shiny::reactiveValues(Diff_table=Diff_table) \n',
             '# Panel sizes \n',
             ...
             ' \n',
             '# Plot violin / boxplot \n',
             'scVioBox <- function(inpConf, inpMeta, inp1, inp2, inpsubg, inpOrder, \n',
             '                     inpsub1, inpsub2, inpH5, inpGene, \n',     
             '                     inptyp, inppts, inpsiz, inpfsz){{ \n',
             '  if(is.null(inpsub1)){{inpsub1 = inpConf$UI[1]}} \n',
             '  use_subg <- inpConf[UI == inpsubg]$ID \n',
             '  # Prepare ggData \n',
             '  ggData = inpMeta[, c(inpConf[UI == inp1]$ID, use_subg, inpConf[UI == inpsub1]$ID), \n',
             '                   with = FALSE] \n',
             '  if(length(use_subg)==0){{ \n',
             '      colnames(ggData) = c("X", "sub")  \n',
             '    }}else{{ \n',
             '      colnames(ggData) = c("X", "subg", "sub")  \n',
             '    }} \n',
             '  \n',
             ...
             '    ggCol = ggCol[ggLvl] \n',
             '  }}else{{ \n',
             '    ggCol = strsplit(inpConf[UI == inpsubg]$fCL, "\\\\|")[[1]]  \n',
             '    names(ggCol) = levels(ggData$subg)  \n',
             '    ggLvl = levels(ggData$subg)[levels(ggData$subg) %in% unique(ggData$subg)]  \n',
             '    ggData$subg = factor(ggData$subg, levels = ggLvl)  \n',
             '    ggCol = ggCol[ggLvl] \n',
             '  }} \n',
             '  \n',
             ' ggData$X <- factor(ggData$X,levels = inpOrder) \n',
             '  # Actual ggplot \n',
             '  if(inptyp == "violin"){{ \n',
             '    if(length(use_subg)==0){{ \n',
             '        ggOut <- ggplot(ggData, aes(X, val, fill = X)) + geom_violin(scale = "width") \n',
             '    }}else{{ \n',
             '        ggOut <- ggplot(ggData, aes(X, val)) + geom_violin(aes(fill=subg),scale = "width") \n',
             '    }}  \n',
             '  }} else {{ \n',
             '    if(length(use_subg)==0){{ \n',
             '        ggOut <- ggplot(ggData, aes(X, val, fill = X)) + geom_boxplot() \n',
             '    }}else{{ \n',
             '        ggOut <- ggplot(ggData, aes(X, val)) + geom_boxplot(aes(fill=subg)) \n',
             '    }}  \n',
             '  }} \n',
             '  if(inppts){{ \n',
             '    if(length(use_subg)==0){{ \n',
             ...
             '    }}else{{ \n',
             '        ggOut = ggOut + geom_jitter(aes(fill = subg), size = inpsiz, shape = 16)  \n',
             '    }} \n',
             '  }} \n',
             '  if(length(use_subg) > 0){{ \n',
             '   if(inp1==inpsubg){{ \n',
             ...
             '           ) %>%  \n',
             '           rstatix::add_xy_position(x = "X") %>%   \n',
             '           dplyr::mutate(p.format=signif(p,2))  \n',
             '         ggOut = ggOut + ggpubr::stat_pvalue_manual(  \n',
             '           stat.test, label = "p.format",   \n',
             '           step.increase = 0, hide.ns = TRUE, tip.length = 0  \n',
             '         ) \n',
             '       }}else{{ \n',
             '      stat.test <- ggData %>% \n',
             '        dplyr::group_by(X) %>% \n',
             '        dplyr::filter(n_distinct(subg) > 1) %>% \n',
             '        rstatix::wilcox_test( \n',
             '          val ~ subg,  \n',
             '          p.adjust.method = "bonferroni" \n',
             '        ) %>% \n',
             ...
             '    ggOut = ggplot(ggData, aes(X, pctCells/100, fill = grp)) + \n',
             '      geom_col() + ylab("Cell Proportion")+ \n',
             '      scale_y_continuous(label = scales::percent) \n',
             '    if(inplabel){{ \n',
             '       ggOut = ggOut + geom_text(aes(label = scales::percent(pctCells/100)), \n',
             '           position="stack",vjust=+2.1,col="black",size=4) \n',
             '    }} \n',
             '  }} else {{ \n',
             '    ggOut = ggplot(ggData, aes(X, nCells, fill = grp)) + \n',
             '      geom_col() + ylab("Number of Cells") \n',
             '    if(inplabel){{ \n',
             '       ggOut = ggOut + geom_text(aes(label = nCells), \n',
             '           position="stack",vjust=+2.1,col="black",size=4) \n',
             '    }} \n',
             '  }} \n',
             '  if(inpflp){{ \n',
             ...
             '}} \n',
             ' \n',
             '# Plot gene expression bubbleplot / heatmap /violinplot \n',
             'scBubbHeat <- function(inpConf, inpMeta, inp, inpGrp, inpOrder, inpSplit, inpSplitOrder, inpPlt, \n',
             ...
             '  shiny::validate(need(nrow(geneList) <= 100, "More than 100 genes to plot! Please reduce the gene list!")) \n',
             ...
             '    if(inpSplit != "Options"){{ \n',
             '      tmp$splitBy = inpMeta[[inpConf[UI == inpSplit]$ID]] \n',
             '    }} \n',
             '    tmp$geneName = iGene \n',
             ...
             '  ggData$grpBy <- factor(ggData$grpBy,levels = inpOrder) \n',
             '  if(inpSplit != "Options"){{ \n',
             '    ggData$splitBy <- factor(ggData$splitBy,levels = inpSplitOrder) \n',
             '  }} \n',
             '  ggData_raw = as.data.frame(ggData) \n',
             '  # Aggregate \n',
             '  # ggData$val = expm1(ggData$val) \n',
             '  if(!inpSplit %in% c("Options", inpGrp)){{ \n',
             '      inpRow = FALSE;inpCol=FALSE \n',
             '      ggData = ggData[, .(val = mean(val), prop = sum(val>0) / length(sampleID)),  \n',
             '                      by = c("geneName", "grpBy", "splitBy")]  \n',
             '    }}else{{ \n',
             '      ggData = ggData[, .(val = mean(val), prop = sum(val>0) / length(sampleID)),  \n',
             '                      by = c("geneName", "grpBy")]  \n',
             '    }} \n',
             ...
             '  if(inpPlt %in% c("Bubbleplot","Heatmap")){{ \n',
             '    if(inpPlt == "Bubbleplot"){{ \n',
             '      # Bubbleplot \n',
             '      if(!inpSplit %in% c("Options", inpGrp)){{ \n',
             '          ggOut = ggplot(ggData, aes(grpBy, geneName, color = val, size = prop)) +  \n',
             '            geom_point() + facet_nested(~ grpBy + splitBy, scales = "free_x") \n',
             '      }}else{{ \n',
             '          ggOut = ggplot(ggData, aes(grpBy, geneName, color = val, size = prop)) +  \n',
             '            geom_point() \n',
             '      }} \n',
             ...
             '             geom_tile() + facet_nested(~ grpBy + splitBy, scales = "free_x") \n',
             '          }}else{{ \n',
             '             ggOut = ggplot(ggData, aes(grpBy, geneName, fill = val)) +  \n',
             '              geom_tile() \n',
             '          }} \n',
             ...
             '      names(ggCol) = levels(ggData_raw$grpBy) \n',
             '      # Violinplot \n',
             '      if(!inpSplit %in% c("Options", inpGrp)){{ \n',
             '            ggOut = ggplot(ggData_raw, aes(grpBy, val,fill=grpBy)) +  \n',
             '              geom_violin(scale = "width", adjust = 1, trim = TRUE) + \n',
             '              facet_nested(geneName ~ grpBy + splitBy, scales = "free_x") \n',
             '      }}else{{ \n',
             '            ggOut = ggplot(ggData_raw, aes(grpBy, val,fill=grpBy))+ \n',
             '              geom_violin(scale = "width", adjust = 1, trim = TRUE) + \n',
             ...
             '}} \n',
             ' \n',
             '# Deferential expression function \n',
             'WilcoxDETest <- function(data.use, cells.1, cells.2) {{ \n',
             ...
             '}} \n',
             ' \n',
             'Load_matrix <- function(inpH5,inpGene){{ \n',
             ...
             '}} \n',
             ' \n',
             'TopMarker <- function(DEG,top_gene=100){{ \n',
             '  TopMarkers <- DEG %>% dplyr::select(-statistic,-auc) %>%  \n',
             '  dplyr::mutate_at(c("pct_in","pct_out"),function(x)round(x,3)) %>% \n',
             '  dplyr::group_by(group) %>% dplyr::arrange(padj, desc(abs(logFC))) %>%  \n',
             '  dplyr::top_n(n=top_gene,wt=logFC) %>% dplyr::ungroup() %>%  \n',
             '  dplyr::arrange(group,padj,desc(abs(logFC))) \n',
             '  return(TopMarkers) \n',
             '}} \n',
             ' \n',
             'FindMarkers <- function(inpConf, inpMeta,inpsub1, inpsub2,group_by = NULL,group.1 = NULL, group.2 = NULL,inpH5,inpGene,use_top_gene=100){{ \n',
             '  if(is.null(inpsub1)){{inpsub1 = inpConf$UI[1]}}  \n',
             '  print("Running FindMarkers...") \n',
             '  X_matrix <- Load_matrix(inpH5,inpGene) \n',
             '  print(sprintf("Dim matrix:%s x %s",dim(X_matrix)[1],dim(X_matrix)[2])) \n',
             '  colnames(X_matrix) <- inpMeta$sampleID \n',
             '  rownames(X_matrix) <- names(sort(inpGene)) \n',
             '  tmp = inpMeta[, c("sampleID", inpConf[UI == inpsub1]$ID), with = FALSE]  \n',
             '  colnames(tmp) = c("sampleID", "sub")  \n',
             '  if(length(inpsub2) != 0 & length(inpsub2) != nlevels(tmp$sub)){{  \n',
             '    tmp <- tmp[sub %in% inpsub2]  \n',
             '    inpMeta <- inpMeta[sampleID %in% tmp$sampleID] \n',
             '  }}  \n',
             '  cells.1 <- inpMeta[get(group_by)==group.1][["sampleID"]] \n',
             '  cells.2 <- inpMeta[get(group_by)==group.2][["sampleID"]] \n',
             '  res <- WilcoxDETest(data.use=X_matrix, cells.1, cells.2) \n',
             '  res$group <- dplyr::recode(res$group,"Group1"=group.1,"Group2"=group.2) \n',
             '  res <- TopMarker(res,top_gene = use_top_gene) %>%  \n',
             '  dplyr::filter(group==group.1) %>% dplyr::select(-group) %>%  \n',
             '  dplyr::rename(!!paste0("pct_",group.1) := pct_in, !!paste0("pct_",group.2) := pct_out) \n',
             '  print("FindMarkers finished") \n',
             '  return(res) \n',
             '}} \n',
             ' \n',
             'FindAllMarkers <- function(inpConf, inpMeta,inpsub1, inpsub2,group_by,inpH5,inpGene,use_top_gene=100){{ \n',
             ...
             '    tmp <- tmp[sub %in% inpsub2]  \n',
             '    inpMeta <- inpMeta[sampleID %in% tmp$sampleID] \n',
             '    X_matrix <- X_matrix[,inpMeta$sampleID] \n',
             '  }} \n',
             '  y <- as.character(inpMeta[[group_by]]) \n',
             '  res <- presto::wilcoxauc(X_matrix, y = y) \n',
             '  res <- TopMarker(res,top_gene = use_top_gene) \n',
             '  print("FindAllMarkers finished") \n',
             '  return(res) \n',
             '}} \n',
             ' \n',
             ' \n',
...
wrSVmain <- function(prefix, subst = "") {
  glue::glue('optCrt="{{ option_create: function(data,escape) {{return(\'<div class=\\"create\\"><strong>\' + \'</strong></div>\');}} }}" \n', 
             ...
             '{subst}                       choices = sub, selected = sub) \n',
             '{subst}  }}) \n',
             '{subst}  output${prefix}c1order.ui <- renderUI({{  \n',
             '{subst}    orderInput(inputId = "{prefix}c1order", label = "Group order:",  \n',
             '{subst}    items = strsplit({prefix}conf[UI == input${prefix}c1inp1]$fID, "\\\\|")[[1]]) \n',
             '{subst}    }}) \n',
             ...
             '    scVioBox({prefix}conf, {prefix}meta, input${prefix}c1inp1, input${prefix}c1inp2, input${prefix}c1grp, input${prefix}c1order, \n',
             ...
             '      plot = scVioBox({prefix}conf, {prefix}meta, input${prefix}c1inp1, input${prefix}c1inp2, input${prefix}c1grp, input${prefix}c1order, \n',
             '                      input${prefix}c1sub1, input${prefix}c1sub2, \n',
             '                      "{prefix}gexpr.h5", {prefix}gene, input${prefix}c1typ, input${prefix}c1pts, \n',
             '                      input${prefix}c1siz, input${prefix}c1fsz) ) \n',
             '  }}) \n',
             ...
             '           input${prefix}c2typ, input${prefix}c2flp, input${prefix}c2label, input${prefix}c2fsz) \n',
             '  }}) \n',
             ...
             '   \n', 
             '  ### Plots for tab d1 \n',
             '{subst} output${prefix}d1order.ui <- renderUI({{ \n',
             '{subst}     orderInput(inputId = "{prefix}d1order", label = "Group order:",  \n',
             '{subst}                items = strsplit({prefix}conf[UI == input${prefix}d1grp]$fID, "\\\\|")[[1]]) \n',
             '{subst}   }}) \n',
             '{subst} output${prefix}d1splitorder.ui <- renderUI({{ \n',
             '{subst}     orderInput(inputId = "{prefix}d1splitorder", label = "Split order:",  \n',
             '{subst}                items = ifelse(input${prefix}d1split=="Options","Waiting...",strsplit({prefix}conf[UI == input${prefix}d1split]$fID, "\\\\|"))[[1]]) \n',
             '{subst}   }}) \n',
             ...
             '    if(nrow(geneList) > 100){{ \n',
             '      HTML("More than 100 input genes! Please reduce the gene list!") \n',
             '    }} else {{ \n',
             ...
             '    scBubbHeat({prefix}conf, {prefix}meta, input${prefix}d1inp, input${prefix}d1grp, input${prefix}d1order, input${prefix}d1split, input${prefix}d1splitorder, input${prefix}d1plt, \n',
             ...
             '      plot = scBubbHeat({prefix}conf, {prefix}meta, input${prefix}d1inp, input${prefix}d1grp, input${prefix}d1order, input${prefix}d1split, input${prefix}d1splitorder, input${prefix}d1plt, \n',
             '                        input${prefix}d1sub1, input${prefix}d1sub2, "{prefix}gexpr.h5", {prefix}gene, \n',
             '                        input${prefix}d1scl, input${prefix}d1row, input${prefix}d1col, \n',
             ...
             '   \n',
             '  ### tables for tab e1 \n',
             '  output${prefix}e1sub1.ui <- renderUI({{   \n',
             '      sub = strsplit({prefix}conf[UI == input${prefix}e1sub1]$fID, "\\\\|")[[1]]   \n',
             '      checkboxGroupInput("{prefix}e1sub2", "Select which cells to show", inline = TRUE,   \n',
             '                         choices = sub, selected = sub)   \n',
             '    }})   \n',
             '    observeEvent(input${prefix}e1sub1non, {{   \n',
             '      sub = strsplit({prefix}conf[UI == input${prefix}e1sub1]$fID, "\\\\|")[[1]]   \n',
             '      updateCheckboxGroupInput(session, inputId = "{prefix}e1sub2", label = "Select which cells to show",   \n',
             '                               choices = sub, selected = NULL, inline = TRUE)   \n',
             '    }})   \n',
             '    observeEvent(input${prefix}e1sub1all, {{   \n',
             '      sub = strsplit({prefix}conf[UI == input${prefix}e1sub1]$fID, "\\\\|")[[1]]   \n',
             '      updateCheckboxGroupInput(session, inputId = "{prefix}e1sub2", label = "Select which cells to show",   \n',
             '                               choices = sub, selected = sub, inline = TRUE)   \n',
             '    }})   \n',
             '  observeEvent(input${prefix}e1grp, {{  \n',
             '      sub = strsplit({prefix}conf[UI == input${prefix}e1grp]$fID, "\\\\|")[[1]]  \n',
             '      updateSelectizeInput(session, inputId = "{prefix}e1grp1",  \n',
             '                               choices = sub, selected = NULL)  \n',
             '      updateSelectizeInput(session, inputId = "{prefix}e1grp2",  \n',
             '                           choices = sub, selected = NULL) \n',
             '    }}) \n',
             '  findmarkers <- shiny::reactive({{  \n',
             '      shiny::withProgress(message="processing",value=0.3,{{  \n',
             '        tryCatch({{  \n',
             '          use_data$Diff_table <- FindMarkers({prefix}conf,{prefix}meta,input${prefix}e1sub1, input${prefix}e1sub2,group_by = input${prefix}e1grp,group.1=input${prefix}e1grp1,group.2=input${prefix}e1grp2,inpH5="{prefix}gexpr.h5",inpGene={prefix}gene,use_top_gene = as.numeric(input$top_gene))   \n',
             '        }})  \n',
             '      }})  \n',
             '    }})  \n',
             '    findallmarkers <- shiny::reactive({{  \n',
             '      shiny::withProgress(message="processing",value=0.3,{{  \n',
             '        tryCatch({{  \n',
             '          use_data$Diff_table <- FindAllMarkers({prefix}conf,{prefix}meta,input${prefix}e1sub1, input${prefix}e1sub2,group_by = input${prefix}e1grp,inpH5="{prefix}gexpr.h5",inpGene={prefix}gene,use_top_gene = as.numeric(input$top_gene))  \n',
             '        }})  \n',
             '      }})  \n',
             '    }})  \n',
             '    observeEvent(input${prefix}e1findmarkers, {{   \n',
             '      findmarkers()  \n',
             '    }})  \n',
             '    observeEvent(input${prefix}e1findallmarkers, {{   \n',
             '      findallmarkers()  \n',
             '    }})  \n',
             '  output${prefix}e1oup.ui <- DT::renderDataTable({{use_data$Diff_table}})  \n',
             '    output${prefix}e1oup.xlsx <- downloadHandler(   \n',
             '      filename = function() {{ paste0("{prefix}_",input${prefix}e1grp,".xlsx") }},   \n',
             '      content = function(f) {{  \n',
             '        if(!is.null(use_data$Diff_table)) {{  \n',
             '          rio::export(use_data$Diff_table, file=f)  \n',
             '        }}  \n',
             '      }})  \n',
             '   \n',
             '   \n',
...
wrUImain <- function(prefix, subst = "", ptsiz = "1.25") {
  glue::glue('  ### Tab1.a1: cellInfo vs geneExpr on dimRed \n',
             ...
             '                           choices = c("White-Red","Lightgrey-Blue","Blue-Yellow-Red","Yellow-Green-Purple"), \n',
             '                           selected = "White-Red"), \n',
             ...
             '       selectInput("{prefix}c1grp", "Compare between (Different analysis):",   \n',
             '                          choices = c("Options",{prefix}conf[grp == TRUE]$UI),    \n',
             '                          selected = "Options") %>%    \n',
             '                helper(type = "inline", size = "m", fade = TRUE,    \n',
             '                       title = "Cell information to subgroup cells by",    \n',
             '                       content = c("Select categorical cell information to subgroup cells by",    \n',
             '                                   "- Single cells are grouped by this categorical covariate")), \n',
             ...
             '      checkboxInput("{prefix}c2label", "Add label", value = FALSE), \n',
             ...
             '  tabPanel( \n',
             '    HTML("Bubbleplot / Heatmap / Violinplot"), \n',
             '    h4("Gene expression bubbleplot / heatmap / violinplot"), \n',
             ...
             '                                          (Max 100 genes, separated <br /> \n',
             '                                           by , or ; or newline):"), \n',
             '                      height = "200px", \n',
             '                      value = paste0({prefix}def$genes, collapse = ", ")) %>% \n',
             '          helper(type = "inline", size = "m", fade = TRUE, \n',
             '                 title = "List of genes to plot on bubbleplot / heatmap / violinplot", \n',
             '                 content = c("Input genes to plot", \n',
             '                             "- Maximum 100 genes (due to ploting space limitations)", \n',
             ...
             '                 content = c("Select categorical cell information to group cells by", \n',
             '                             "- Single cells are grouped by this categorical covariate", \n',
             '                             "- Plotted as the X-axis of the bubbleplot / heatmap / violinplot")), \n',
             '        uiOutput("{prefix}d1order.ui"), \n',
             '        br(), \n',
             '        selectInput("{prefix}d1split", "Split by:",  \n',  
             '                 choices = c("Options",{prefix}conf[grp == TRUE]$UI),     \n',
             '                 selected = "Options") %>%     \n',
             '          helper(type = "inline", size = "m", fade = TRUE,     \n',
             '                 title = "Cell information to split cells by",     \n',
             '                 content = c("Select categorical cell information to split cells by",     \n',
             '                             "- Single cells are splited by this categorical covariate")), \n',
             '        uiOutput("{prefix}d1splitorder.ui"), \n',
             '        br(), \n',
             ...
             '  ),      # End of tab (2 space) \n',
             '   \n',
             '    ### Tab1.e1: gene differential expr  \n',
             '      tabPanel(  \n',
             '        HTML("Differential expression"),  \n',
             '        h4("Gene differential expression"),  \n',
             '        "In this tab, users can perform gene differential expression analysis ",  \n',
             '        "grouped by categorical cell information (e.g. library / cluster) or using partial cells (group.1 and group.2).", br(),  \n',
             '        br(),br(),  \n',
             '        fluidRow(  \n',
             '          column(  \n',
             '            3, style="border-right: 2px solid black",  \n',
             '            actionButton("{prefix}e1togL", "Toggle to subset cells"),   \n',
             '                        conditionalPanel(   \n',
             '                          condition = "input.{prefix}e1togL % 2 == 1",   \n',
             '                          selectInput("{prefix}e1sub1", "Cell information to subset:",   \n',
             '                                      choices = {prefix}conf[grp == TRUE]$UI,   \n',
             '                                      selected = {prefix}def$grp1),   \n',
             '                          uiOutput("{prefix}e1sub1.ui"),   \n',
             '                          actionButton("{prefix}e1sub1all", "Select all groups", class = "btn btn-primary"),   \n',
             '                          actionButton("{prefix}e1sub1non", "Deselect all groups", class = "btn btn-primary")   \n',
             '                        ), br(), br(),  \n',
             '            selectInput("{prefix}e1grp", "Group by:",  \n',
             '                        choices = {prefix}conf[grp == TRUE]$UI,   \n',
             '                        selected = {prefix}conf[grp == TRUE]$UI[1]) %>%   \n',
             '              helper(type = "inline", size = "m", fade = TRUE,   \n',
             '                     title = "Cell information to group cells by",   \n',
             ...
             '                                 "- Single cells are grouped by this categorical covariate")),   \n',
             '              \n',
             '            actionButton("{prefix}e1findallmarkers", "FindAllMarkers", class = "btn btn-primary"),  \n',
             '            br(),  \n',
             '            br(),  \n',
             '            selectInput("{prefix}e1grp1", "Group 1:",   \n',
             '                        choices = NULL,   \n',
             '                        selected = NULL) %>%   \n',
             '              helper(type = "inline", size = "m", fade = TRUE,   \n',
             '                     title = "Cell information to group cells",   \n',
             '                     content = c("Cell group 1")),   \n',
             '            selectInput("{prefix}e1grp2", "Group 2:",   \n',
             '                        choices = NULL,   \n',
             '                        selected = NULL) %>%   \n',
             '              helper(type = "inline", size = "m", fade = TRUE,   \n',
             '                     title = "Cell information to group cells",   \n',
             ...
             '                     content = c("Default top 100")),  \n',
             '            actionButton("{prefix}e1findmarkers", "FindMarkers", class = "btn btn-primary")  \n',
             '          ), # End of column (6 space)   \n',
             '          column(9, DT::dataTableOutput("{prefix}e1oup.ui"),   \n',
             '                 downloadButton("{prefix}e1oup.xlsx", "Download Table")  \n',
             '          )  # End of column (6 space)   \n',
             '        )    # End of fluidRow (4 space)   \n',
             '      )      # End of tab (2 space)  \n',
             '   \n',    
